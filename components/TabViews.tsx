...
export const PlanningView: React.FC<ViewProps> = ({ appId, highlightId, highlightSeq }) => {
  ...
  const [planningPasteHint, setPlanningPasteHint] = useState(false);
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={() => {
                const next = restructureContentAsChatPaste(editForm.content || '');
                setEditForm({ ...editForm, content: next });
                setPlanningPasteHint(true);
                setTimeout(() => setPlanningPasteHint(false), 2500);
              }}
              className="px-4 py-2 bg-amber-50 border border-amber-300 text-amber-900 hover:bg-amber-100 rounded-lg text-sm transition-colors w-full sm:w-auto"
            >
              구조화 적용
            </button>
            <button
              ...
              <div>
                ...
                {planningPasteHint && (
                  <p className="text-xs text-green-700 font-medium mb-2">
                    채팅 붙여넣기 구조화가 적용되었습니다. 오른쪽 미리보기를 확인하세요.
                  </p>
                )}
        ...
        <div ref={highlightContainerRef} className="overflow-auto flex-1">
          {loading ? <Loading /> : (
            ...
            <div className="lg:hidden p-3 space-y-3">
              ...
              {orderedPrompts.length === 0 && <div className="text-center py-12 text-slate-400 text-sm">로그가 없습니다.</div>}
            </div>
            <table className="hidden lg:table report-table-separators min-w-full divide-y divide-slate-200" style={{ tableLayout: 'fixed', width: '100%' }}>
              <colgroup>
                {resize.widths.map((_, i) => <col key={i} style={resize.getColStyle(i)} />)}
        ...
        <div ref={highlightContainerRef} className="overflow-auto flex-1">
          {loading ? <Loading /> : (
            ...
            <div className="lg:hidden p-3 space-y-3">
              ...
              {orderedMemos.length === 0 && <div className="text-center py-12 text-slate-400 text-sm">작성된 참고가 없습니다.</div>}
            </div>
            <table className="hidden lg:table report-table-separators min-w-full divide-y divide-slate-200" style={{ tableLayout: 'fixed', width: '100%' }}>
              <colgroup>
                {resize.widths.map((_, i) => <col key={i} style={resize.getColStyle(i)} />)}
        ...
        <div ref={highlightContainerRef} className="overflow-auto flex-1">
          {loading ? <Loading /> : (
            ...
            <div className="lg:hidden p-3 space-y-3">
              ...
              {orderedIssues.length === 0 && <div className="text-center py-12 text-slate-400 text-sm">등록된 이슈가 없습니다.</div>}
            </div>
            <table className="hidden lg:table report-table-separators min-w-full divide-y divide-slate-200" style={{ tableLayout: 'fixed', width: '100%' }}>
              <colgroup>
                {resize.widths.map((_, i) => <col key={i} style={resize.getColStyle(i)} />)}
