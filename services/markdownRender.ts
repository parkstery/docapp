...
const MARKDOWN_ALLOWED_ATTR = [
  ...
  'type', 'disabled', 'checked', 'colspan', 'rowspan',
] as const;
...
function isLikelyHtmlDocument(content: string): boolean {
  ...
  if (!t) return false;
  return /<(p|div|h[1-6]|ul|ol|table|blockquote|span|article)\b/i.test(t);
}
  ...
  <head>
    ...
    <style>
      ...
      @media (min-width: 768px) {
        .markdown-body { padding: 32px 24px 48px; }
      }
    </style>
  </head>
  <body>
 ...
 * 기획서(.md) · 보고서 요약(HTML) 모두 동일 스타일로 표시.
 */
export function openDocumentPreviewInBrowser(content: string, title: string): void {
  ...
  const bodyHtml = isLikelyHtmlDocument(trimmed)
    ? sanitizeRichDocumentHtml(trimmed)
    : renderMarkdownForDisplay(trimmed);
...
export function openMarkdownInBrowser(markdownText: string, title: string): void {
  openDocumentPreviewInBrowser(markdownText, title);
}
